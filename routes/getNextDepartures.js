        ...
        if(error){
            ...
            xml2js.parseString(response.body, (err, res) => {
                if (err) {
                    ...
                    result.send(object.parseDeparturesBoard(res, 'GetNextDeparturesResponse'));
                }
            });
...
function getNextDepartures(crs, filterList, timeOffset = 0, timeWindow = 120, res) {

    //Check the parameters
    let errs = check.parameterCheckerDepartures(crs, filterList, timeOffset, timeWindow);
    if(errs.length === 0){
...
router.get('/:crs/:filterList', function (req, res, next) {
    ...
    getNextDepartures(crs, filterList, timeOffset, timeWindow, res);
