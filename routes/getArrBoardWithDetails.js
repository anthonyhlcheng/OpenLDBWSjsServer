        ...
        if(error){
            ...
            xml2js.parseString(response.body, (err, res) => {
                if (err) {
                    ...
                    result.send(object.parseStationBoardWithDetails(res, 'GetArrBoardWithDetailsResponse'));
                }
            });
...
function getArrBoardWithDetails(numRows, crs, filterCrs = null, filterType = "to", timeOffset = 0, timeWindow = 120, res) {
...
router.get('/:numRows/:crs', function (req, res, next) {
    ...
    getArrBoardWithDetails(numRows, crs, filterCrs, filterType, timeOffset, timeWindow, res);
});
