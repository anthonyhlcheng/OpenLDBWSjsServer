        ...
        if(error){
            ...
            xml2js.parseString(response.body, (err, res) => {
                if (err) {
                    ...
                    result.send(object.parseStationBoard(res, 'GetArrivalDepartureBoardResponse'));
                }
            });
...
function getArrivalDepartureBoard(numRows, crs, filterCrs = null, filterType = "to", timeOffset = 0, timeWindow = 120, res) {
...
router.get('/:numRows/:crs', function (req, res, next) {
    ...
    getArrivalDepartureBoard(numRows, crs, filterCrs, filterType, timeOffset, timeWindow, res);
