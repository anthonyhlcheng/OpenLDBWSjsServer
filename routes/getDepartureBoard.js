           ...
           //result.send(response.body);
            ...
            xml2js.parseString(response.body, (err, res) => {
                if (err) {
                    ...
                    result.send(object.parseStationBoard(res, 'GetDepartureBoardResponse'));
                }
            });
...
function getDepartureBoard(numRows, crs, filterCrs = null, filterType = "to", timeOffset = 0, timeWindow = 120, res) {
...
router.get('/:numRows/:crs', function (req, res, next) {
    ...
    getDepartureBoard(numRows, crs, filterCrs, filterType, timeOffset, timeWindow, res);
