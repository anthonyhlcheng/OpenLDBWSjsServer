        ...
        if(error){
            ...
            xml2js.parseString(response.body, (err, res) => {
                if (err) {
                    ...
                    result.send(object.parseDeparturesBoard(res, 'GetFastestDeparturesResponse'));
                }
            });
...
function getFastestDepartures(crs, filterList, timeOffset = 0, timeWindow = 120, res) {

    //Check the parameters
    let errs = check.parameterCheckerFastestDepartures(crs, filterList, timeOffset, timeWindow);
    if(errs.length === 0){
...
router.get('/:crs/:filterList', function (req, res, next) {
    ...
    getFastestDepartures(crs, filterList, timeOffset, timeWindow, res);
