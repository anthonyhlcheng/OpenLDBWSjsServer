        ...
        if(error){
            ...
            xml2js.parseString(response.body, (err, res) => {
                if (err) {
                    ...
                    result.send(object.parseDeparturesBoardWithDetails(res, 'GetNextDeparturesWithDetailsResponse'));
                }
            });
...
function getNextDeparturesWithDetails(crs, filterList, timeOffset = 0, timeWindow = 120, res) {
...
router.get('/:crs/:filterList', function (req, res, next) {
    ...
    getNextDeparturesWithDetails(crs, filterList, timeOffset, timeWindow, res);
